...
public abstract class SMTPAppenderBase<E> extends AppenderBase<E> {
    ...
    public void start() {
        ...
        if (session == null) {
            addError("Failed to obtain jakarta.mail.Session. Cannot start.");
            return;
        }
    ...
    private Session lookupSessionInJNDI() {
        addInfo("Looking up jakarta.mail.Session at JNDI location [" + jndiLocation + "]");
        try {
            Context initialContext = JNDIUtil.getInitialContext();
            Object obj = JNDIUtil.lookupObject(initialContext, jndiLocation);
            return (Session) obj;
        } catch (Exception e) {
            addError("Failed to obtain jakarta.mail.Session from JNDI location [" + jndiLocation + "]");
            return null;
        }
    }
    ...
    /**
     * Set the location where a {@link jakarta.mail.Session} resource is located in JNDI. Default value is
    ...
    /**
     * If set to true, a {@link jakarta.mail.Session} resource will be retrieved from JNDI. Default is false.
     *
     * @param sessionViaJNDI whether to obtain a jakarta.mail.Session by JNDI
     * @since 1.0.6
     */
